        ...
        <div class="form-group form-group-default">
            ...
            <select name="jadwal" id="jadwal" class="form-control custom-select full-width" wire:model='jadwal_tes' required> 
                ...
                @foreach ($daftar_jadwals as $daftar_jadwal)
                    <option value="{{ $daftar_jadwal->id }}"><b>Tanggal: </b>{{ $daftar_jadwal->tanggal_tes }} <b>Waktu: </b>{{ date('H:i', strtotime($daftar_jadwal->waktu_mulai_akses)) }} - {{ date('H:i', strtotime($daftar_jadwal->waktu_selesai_akses)) }} WITA</option>
                @endforeach
            </select>
        </div>
